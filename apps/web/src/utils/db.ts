import type { Message, Conversation } from "../types/shared";
    ...
    return new Promise((resolve, reject) => {
      ...
      request.onupgradeneeded = (event) => {
        ...
        if (!database.objectStoreNames.contains("messages")) {
          ...
          messageStore.createIndex("sequenceId", "sequenceId", {
            unique: false,
          });
        }
      ...
      try {
        ...
        request.onsuccess = () => {
          ...
          const unsent = list.filter(
            (m) => m.isPending || (m.status === "sent" && m.sequenceId > 1e11),
          );
          resolve(unsent);
        };
        request.onerror = () => reject(request.error);
